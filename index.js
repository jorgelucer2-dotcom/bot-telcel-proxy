...
async function ejecutarFlujoSegunSesion(ctx, id, s) {
    s.paso = 'ejecutando';
    sesiones.set(id, s);
    if (s.modo === 'telcel_prueba' || s.tipo === 'Telcel Prueba') {
        return flujoTelcelPrueba(ctx, id, s).catch(err => console.error(`[Telcel Prueba Usuario ${id}] Error:`, err.message || err));
    }
...
function clasificarResultadoTelcel(textoCompleto, id) {
    ...
    // Incluye la pantalla de Telcel: "Estamos procesando tu pago".
    const esProcesando = /(estamos\s*procesando\s*tu\s*(pago|recarga|solicitud))|(tu\s*recarga\s*est[áa]\s*en\s*proceso)|(recarga\s*en\s*proceso)|(pago\s*en\s*proceso)|(procesando\s*tu\s*(pago|recarga|solicitud))|(espera\s*un\s*momento)|(espera\s*la\s*confirmaci[óo]n)|(validando\s*transacci[óo]n)|(en\s*validaci[óo]n)|(transacci[óo]n\s*en\s*proceso)|(transacci[óo]n\s*pendiente)/i.test(txt);
    if (esProcesando) {
        return {
            estado: 'PROCESANDO',
            subtipo: 'EN_PROCESO',
    ...
    try {
        await ejecutarConReintento(async (intento) => {
            ...
            try {
                ...
                logTelcel(id, `⏳ Esperando confirmación/rechazo definitivo del portal (máx 180s)...`);
                const inicioEspera = Date.now();
                const TIEMPO_MAXIMO_ESPERA_TELCEL_MS = 180000;
                let textoFinalPagina = "";
                let clasificacionFinal = null;
                let ultimoEstadoLog = null;

                let vioProcesandoTelcel = false;

                while (Date.now() - inicioEspera < TIEMPO_MAXIMO_ESPERA_TELCEL_MS) {
                    // Leer la página principal, popups y TODOS los frames/iframes.
                    // Telcel puede mostrar "Estamos procesando tu pago" dentro de un iframe;
                    // si sólo se lee la página principal, el estado termina como DESCONOCIDO.
                    ...
                    for (const pActual of paginasContexto) {
                        ...
                        for (const frameActual of framesActuales) {
                            try {
                                ...
                                if (textoFrame && textoFrame.trim()) {
                                    fragmentosTotales.push(textoFrame.trim());
                                }
                            } catch (_) {}
                        }
                    }
                ...
                // Si en cualquier momento se observó "procesando", jamás degradarlo a DESCONOCIDO.
                if (
                    vioProcesandoTelcel &&
                    (!clasificacionFinal || ['DESCONOCIDO', 'PROCESANDO'].includes(clasificacionFinal.estado))
                ) {
                    clasificacionFinal = {
                        estado: 'PROCESANDO',
                        subtipo: 'EN_PROCESO',
                        titulo: '⏳ RECARGA EN PROCESO — PENDIENTE DE CONFIRMACIÓN',
                        icono: '⏳',
                        explicacion: 'Telcel muestra "Estamos procesando tu pago". La operación sigue pendiente de confirmación final.'
                    };
                }
                ...
                if (miId === ejecucionesUsuario.get(id)) {
                    ...
                    const capturaVoucher = clasificacionFinal.estado === 'PROCESANDO' ? null : await tomarCapturaEnfocada(pagina);
                    ...
                    if (clasificacionFinal.estado === 'EXITO') {
                        ...
                        // Mientras Telcel siga en "Estamos procesando tu pago",
                        // no se envía captura final ni se trata como rechazo/éxito.
                        mensajeTelegram = 
                            `⏳ <b>RECARGA EN PROCESO — PENDIENTE DE CONFIRMACIÓN</b>\n` +
                            `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
                            `📱 <b>Línea:</b> <code>${numero}</code>\n` +
                            `💲 <b>Monto:</b> $${monto} MXN\n` +
                            `📄 <b>Estado detectado:</b> <i>Estamos procesando tu pago</i>\n\n` +
                            `ℹ️ Telcel no emitió todavía una aprobación ni un rechazo definitivo.\n` +
                            `⚠️ <b>No realices otro intento hasta tener confirmación final.</b>`;
...
// ==============================================================================
// 🧪 TELCEL PRUEBA — RECARGA AMIGO (BLOQUE INDEPENDIENTE / PRE-CHECKOUT)
// URL nueva en prueba. No reemplaza todavía TELCEL.COM / TIENDA TELCEL.
// Este bloque automatiza selección de paquete, captura/confirmación del número
// y apertura de la forma de pago con tarjeta. Se detiene antes de capturar o
// enviar datos de tarjeta/pago para poder validar el flujo nuevo sin afectar
// los otros módulos.
// ==============================================================================
const URL_TELCEL_PRUEBA = 'https://www.telcel.com/personas/amigo/recargas/recarga-amigo';
const navegadoresTelcelPrueba = new Map();

function logTelcelPrueba(id, msg) {
    console.log(`[Telcel Prueba Usuario ${id}] ${msg}`);
}

async function cerrarNavegadorTelcelPrueba(id) {
    if (!id) {
        for (const [userId, nav] of navegadoresTelcelPrueba) {
            try { await nav.close().catch(() => {}); } catch (_) {}
            liberarSlotChromiumLocal(nav, `TELCEL_PRUEBA:${userId}`);
        }
        navegadoresTelcelPrueba.clear();
        return;
    }
    const nav = navegadoresTelcelPrueba.get(id);
    navegadoresTelcelPrueba.delete(id);
    if (!nav) return;
    try { await nav.close().catch(() => {}); } catch (_) {}
    liberarSlotChromiumLocal(nav, `TELCEL_PRUEBA:${id}`);
}

async function crearNavegadorTelcelPrueba(id) {
    await cerrarNavegadorTelcelPrueba(id);
    const browser = await lanzarChromiumLocalControlado({
        headless: ES_HEADLESS,
        timeout: 35000,
        args: [
            '--no-sandbox',
    ...
    navegadoresTelcelPrueba.set(id, browser);
    const context = await browser.newContext({
        locale: 'es-MX',
        timezoneId: 'America/Mexico_City',
        viewport: { width: 1280, height: 900 }
...
async function flujoTelcelPrueba(ctx, id, datos) {
    const monto = Number(datos.monto || 200);
    const numero = String(datos.numero || '');
    const montosPermitidos = new Set([200, 300, 500]);
    if (!montosPermitidos.has(monto)) {
        throw new Error(`TELCEL_PRUEBA_MONTO_${monto}_NO_PERMITIDO`);
    }
    let sesion = null;

    try {
        await ctx.reply(
            `🧪 <b>TELCEL PRUEBA — INICIANDO</b>\n` +
            `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
            `📱 Línea: <code>${numero}</code>\n` +
            `💲 Monto: $${monto} MXN\n\n` +
            `⏳ Probando la nueva página de Recarga Amigo...`,
            { parse_mode: 'HTML' }
        ).catch(() => {});

        sesion = await crearNavegadorTelcelPrueba(id);
        const { page } = sesion;

        logTelcelPrueba(id, `🌐 Abriendo ${URL_TELCEL_PRUEBA}`);
        await page.goto(URL_TELCEL_PRUEBA, { waitUntil: 'domcontentloaded', timeout: 45000 });

        // Cookies/avisos: solo si aparecen; no son requisito para seguir.
        for (const sel of ['#onetrust-accept-btn-handler', 'button:has-text("Aceptar todas")', 'button:has-text("Aceptar")']) {
        ...
        // PASO A: localizar dinámicamente Recarga 200 / 300 / 500.
        // No depende de una posición fija: inspecciona las tarjetas y hace scroll
        // hasta que el paquete solicitado exista en DOM y quede visible.
        logTelcelPrueba(id, `🔎 Buscando paquete Recarga ${monto}`);

        const regexPaquete = new RegExp(`Recarga\\s*${monto}(?:\\D|$)`, 'i');
        const tarjetasPaquete = page.locator('div.packages-grid__item');
        await tarjetasPaquete.first().waitFor({ state: 'attached', timeout: 15000 }).catch(() => {});

        let tarjetaPaquete = tarjetasPaquete.filter({ hasText: regexPaquete }).first();
        let paqueteVisible = false;
        let sinAvance = 0;
        let ultimoScrollTop = -1;

        for (let intentoScroll = 1; intentoScroll <= 30; intentoScroll++) {
            const existeEnDom = (await tarjetaPaquete.count().catch(() => 0)) > 0;

            if (existeEnDom) {
                logTelcelPrueba(id, `✅ Recarga ${monto} localizada en DOM`);
                await tarjetaPaquete.scrollIntoViewIfNeeded().catch(() => {});
                await page.waitForTimeout(250);
                paqueteVisible = await tarjetaPaquete.isVisible({ timeout: 2000 }).catch(() => false);
                if (paqueteVisible) break;
            }

            const estadoScroll = await page.evaluate(() => {
                const el = document.scrollingElement || document.documentElement;
                const paso = Math.max(500, Math.floor(window.innerHeight * 0.82));
                const antes = el.scrollTop;
                window.scrollBy(0, paso);
                return {
                    antes,
                    despues: el.scrollTop,
                    alto: el.scrollHeight,
                    viewport: window.innerHeight
                };
            }).catch(() => null);

            logTelcelPrueba(id, `⬇️ Scroll de búsqueda ${intentoScroll}/30 para localizar Recarga ${monto}`);
            await page.waitForTimeout(350);

            // El sitio puede lazy-renderizar más tarjetas tras el scroll.
            tarjetaPaquete = tarjetasPaquete.filter({ hasText: regexPaquete }).first();

            if (estadoScroll) {
                if (estadoScroll.despues === ultimoScrollTop || estadoScroll.despues === estadoScroll.antes) sinAvance++;
                else sinAvance = 0;
                ultimoScrollTop = estadoScroll.despues;

                const enFondo = estadoScroll.despues + estadoScroll.viewport >= estadoScroll.alto - 4;
                if (enFondo && sinAvance >= 2) {
                    logTelcelPrueba(id, `ℹ️ Se alcanzó el final de la lista buscando Recarga ${monto}`);
                    break;
                }
            }
        }
        ...
        if (!paqueteVisible) {
            // Diagnóstico corto de las tarjetas realmente renderizadas.
            const textos = await tarjetasPaquete.evaluateAll(els =>
                els.map(el => (el.innerText || '').replace(/\s+/g, ' ').trim()).filter(Boolean).slice(0, 20)
            ).catch(() => []);
            logTelcelPrueba(id, `🧾 Paquetes visibles/renderizados: ${textos.join(' || ').slice(0, 1200) || 'SIN_TEXTO'}`);
            throw new Error(`TELCEL_PRUEBA_PAQUETE_${monto}_NO_VISIBLE`);
        }

        await tarjetaPaquete.scrollIntoViewIfNeeded().catch(() => {});
        logTelcelPrueba(id, `✅ Tarjeta Recarga ${monto} visible`);

        const botonLoQuiero = tarjetaPaquete.getByRole('button', { name: 'Lo quiero', exact: true }).first();
        await botonLoQuiero.waitFor({ state: 'visible', timeout: 10000 });
        await botonLoQuiero.scrollIntoViewIfNeeded().catch(() => {});
        await botonLoQuiero.click();
        logTelcelPrueba(id, `✅ Paquete $${monto} seleccionado`);

        // PASO B: número y confirmación con selectores reales confirmados.
        const mdn = page.locator('input[name="mdn"]');
        const confirmMdn = page.locator('input[name="confirmMdn"]');
        await mdn.waitFor({ state: 'visible', timeout: 15000 });
        await mdn.fill(numero);
        await confirmMdn.fill(numero);
        await confirmMdn.blur().catch(() => {});
        logTelcelPrueba(id, '✅ Número y confirmación escritos');

        const continuar = page.locator('button[type="submit"][aria-label="Continuar a confirmación de datos"]');
        await continuar.waitFor({ state: 'visible', timeout: 10000 });

        const limiteBoton = Date.now() + 12000;
        while (Date.now() < limiteBoton) {
            const habilitado = await continuar.isEnabled().catch(() => false);
            if (habilitado) break;
            await page.waitForTimeout(250);
        }
        if (!(await continuar.isEnabled().catch(() => false))) {
            throw new Error('TELCEL_PRUEBA_CONTINUAR_NO_HABILITADO');
        }
        await continuar.click();
        logTelcelPrueba(id, '➡️ Continuando a forma de pago');

        // PASO C: forma de pago CARD con selector real confirmado.
        const radioCard = page.locator('input[type="radio"][name="paymentType"][value="CARD"]');
        await radioCard.waitFor({ state: 'attached', timeout: 20000 });
        if (!(await radioCard.isChecked().catch(() => false))) {
            await radioCard.check({ force: true });
        }
        logTelcelPrueba(id, '✅ Forma de pago CARD seleccionada');

        // Confirmar que el formulario de tarjeta ya apareció, sin capturar ni enviar datos sensibles.
        const cardNumber = page.locator('input[name="cardNumber"]');
        await cardNumber.waitFor({ state: 'visible', timeout: 15000 });
        logTelcelPrueba(id, '✅ Formulario de pago visible — prueba pre-checkout completada');

        const captura = await page.screenshot({ fullPage: false }).catch(() => null);
        const caption =
            `🧪 <b>TELCEL PRUEBA — PRE-CHECKOUT OK</b>\n` +
            `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
            `✅ Paquete $${monto} localizado\n` +
            `✅ Número capturado y confirmado\n` +
            `✅ Forma de pago CARD localizada\n` +
            `✅ Formulario de tarjeta visible\n\n` +
            `🛑 La prueba se detuvo antes de capturar/enviar datos de tarjeta o confirmar un pago.`;

        if (captura) {
            await ctx.replyWithPhoto({ source: captura }, { caption: caption.replace(/<[^>]+>/g, '') }).catch(() => {});
        } else {
            await ctx.reply(caption, { parse_mode: 'HTML' }).catch(() => {});
        }

        sesiones.delete(id);
        return { ok: true, etapa: 'PRE_CHECKOUT' };
    } catch (error) {
        const msg = String(error?.message || error);
        logTelcelPrueba(id, `❌ ${msg}`);
        if (sesion?.page && !sesion.page.isClosed()) {
            const captura = await sesion.page.screenshot({ fullPage: false }).catch(() => null);
            if (captura) {
                await ctx.replyWithPhoto({ source: captura }, { caption: `🧪 TELCEL PRUEBA — ERROR\n${msg.slice(0, 180)}` }).catch(() => {});
            }
        }
        await ctx.reply(
            `❌ <b>TELCEL PRUEBA — ERROR</b>\n\n<code>${msg.slice(0, 220)}</code>`,
            { parse_mode: 'HTML' }
        ).catch(() => {});
        return { ok: false, error: msg };
    } finally {
        await cerrarNavegadorTelcelPrueba(id).catch(() => {});
    }
}
...
async function mostrarMenuInicio(ctx, esReinicio = false) {
    ...
    const textoMenu = 
        ...
        `✅ <b>TELCEL PRUEBA:</b> ▫️$200 ▫️$300 ▫️$500\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
        `📋 <b>CÓMO FUNCIONA</b>\n` +
    ...
    const filasTeclado = [
        ...
        [
            ...
            Markup.button.callback('🧪 TELCEL PRUEBA', 'btn_telcel_prueba')
        ],
        [
...
bot.command(['telcelprueba', 'pruebatelcel'], async ctx => {
    return iniciarCompania(ctx, 'Telcel Prueba');
});
...
bot.action('btn_telcel_prueba', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    return iniciarCompania(ctx, 'Telcel Prueba');
});
...
async function iniciarCompania(ctx, compania) {
    ...
    } else if (compania === 'Telcel Prueba') {
        cerrarNavegadorTelcelPrueba(id).catch(() => {});
    } else {
        cerrarSesionNavegador(id);
    }

    const modo = (compania === 'Telcel.com' || compania === 'Telcel Tienda')
        ? 'telcel_tienda'
        : (compania === 'Telcel Prueba' ? 'telcel_prueba' : compania.toLowerCase());
    ...
    if (compania === 'Telcel Prueba') icono = '🧪';
...
bot.on('text', async (ctx, next) => {
    ...
    if (/^(telcel|bait|tienda|telcel\.com|recargas|telcel prueba|prueba)$/i.test(txt)) {
        let comp = 'Telcel';
        ...
        if (txt.toLowerCase() === 'prueba' || txt.toLowerCase().includes('telcel prueba')) comp = 'Telcel Prueba';
    ...
    if (matchMonto && [200, 230, 300, 500].includes(parseInt(matchMonto[1], 10))) {
        ...
        if ((s.tipo === 'Telcel' || s.tipo === 'Telcel.com' || s.tipo === 'Telcel Prueba') && ![200, 300, 500].includes(monto)) {
            return enviarLimpio(ctx,
        ...
        if (compania === 'Bait') icono = '🔵';
        if (compania === 'Telcel.com' || compania === 'Telcel Tienda' || s.modo === 'telcel_tienda') icono = '🟣';
        if (compania === 'Telcel Prueba' || s.modo === 'telcel_prueba') icono = '🧪';
        if (compania === 'Telcel Prueba' || s.modo === 'telcel_prueba') icono = '🧪';
    ...
    if (s.paso === 'numero' || (/^\d+$/.test(txt) && !s.numero && !String(s.paso || '').startsWith('monto'))) {
        ...
        } else if (s.modo === 'telcel_prueba' || s.tipo === 'Telcel Prueba') {
            nombreFlujo = 'TELCEL PRUEBA';
        }

        console.log(`📱 [Usuario ${id}] Nuevo número recibido: ${txt}`);
        console.log(`🔄 [Usuario ${id}] Reiniciando flujo [${nombreFlujo}]`);

        // TELCEL PRUEBA: bloque independiente de validación hasta pre-checkout.
        // No solicita ni procesa datos de tarjeta en este modo de prueba.
        if (nombreFlujo === 'TELCEL PRUEBA') {
            s.paso = 'ejecutando';
            sesiones.set(id, s);
            return flujoTelcelPrueba(ctx, id, s).catch(err => {
                console.error(`[Telcel Prueba Usuario ${id}] Error:`, err.message || err);
            });
        }
