...
const TIMEOUT_CONFIRMACION_PAGO_MS = Number(process.env.TIMEOUT_CONFIRMACION_PAGO_MS || 300000); // 5 min
const LECTURAS_EXITO_ESTABLE = 2;
const LECTURAS_ERROR_ESTABLE = 3;
    ...
    try {
        await ejecutarConReintento(async (intento) => {
            ...
            try {
                ...
                logTelcel(id, `⏳ Esperando confirmación/respuesta del portal (máx ${Math.round(TIMEOUT_CONFIRMACION_PAGO_MS / 1000)}s)...`);
                const inicioEspera = Date.now();
                const TIEMPO_MAXIMO_ESPERA_TELCEL_MS = TIMEOUT_CONFIRMACION_PAGO_MS;
                let textoFinalPagina = "";
                let clasificacionFinal = null;
                let ultimoEstadoLog = null;
                let estadoCandidato = null;
                let lecturasCandidato = 0;
                ...
                while (Date.now() - inicioEspera < TIEMPO_MAXIMO_ESPERA_TELCEL_MS) {
                    ...
                    // Un mensaje intermedio NO cierra el proceso.
                    // Un estado terminal debe repetirse varias lecturas para evitar falsos positivos.
                    const terminales = ['EXITO', 'RECHAZO_BANCARIO', 'BLOQUEO_TELCEL', 'ERROR_TELCEL'];
                    if (terminales.includes(clasificacionFinal.estado)) {
                        if (estadoCandidato === clasificacionFinal.estado) {
                            lecturasCandidato++;
                        } else {
                            estadoCandidato = clasificacionFinal.estado;
                            lecturasCandidato = 1;
                        }

                        const requeridas = clasificacionFinal.estado === 'EXITO'
                            ? LECTURAS_EXITO_ESTABLE
                            : LECTURAS_ERROR_ESTABLE;

                        logTelcel(id, `🧭 Confirmando estado ${clasificacionFinal.estado}: ${lecturasCandidato}/${requeridas}`);
                        if (lecturasCandidato >= requeridas) break;
                    } else {
                ...
                if (miId === ejecucionesUsuario.get(id)) {
                    ...
                    if (clasificacionFinal.estado === 'EXITO') {
                        const sExito = sesiones.get(id) || datos || {};
                        sExito.paso = 'resultado_exito';
                        sExito.ultimoResultado = 'EXITO';
                        sesiones.set(id, sExito);
                        ...
                        mensajeTelegram = 
                            ...
                            `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
                            `👉 <b>¿Deseas realizar otra recarga con el mismo método de pago?</b>
` +
                            `Si eliges Sí, solo te pediré el nuevo número; se conservarán los demás datos de la recarga exitosa.`;

                        const tecladoExito = sExito.guiadoRecarga
                            ? Markup.inlineKeyboard([
                                [Markup.button.callback('✅ SÍ — MISMO MÉTODO DE PAGO', 'rec_otra_mismos_datos')],
                        ...
                        if (sPrev.guiadoRecarga) {
                            const tarjetas = obtenerTarjetasSesion(id);
                            const indiceActual = Number.isInteger(sPrev.tarjetaSeleccionada) ? sPrev.tarjetaSeleccionada : 0;
                            const siguienteIndice = indiceActual + 1;
                            const siguiente = tarjetas[siguienteIndice];

                            sPrev.paso = 'resultado_rechazado';
                            sPrev.siguienteTarjetaIndice = siguiente ? siguienteIndice : null;
                            sesiones.set(id, sPrev);

                            const filas = [];
                            if (siguiente) {
                                filas.push([Markup.button.callback(
                                    `▶️ CONTINUAR CON TARJETA ${siguienteIndice + 1} •••• ${siguiente.ult4}`,
                                    'rec_confirmar_siguiente_tarjeta'
                                )]);
                            }
                            filas.push([Markup.button.callback('🔄 EMPEZAR DESDE CERO', 'btn_reiniciar')]);
                            filas.push([Markup.button.callback('🦁 MENÚ PRINCIPAL', 'btn_reiniciar')]);

                            mensajeTelegram =
                                `❌ <b>RECARGA NO COMPLETADA</b>\n` +
                                `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
                                `📱 <b>Línea:</b> <code>${numero}</code>\n` +
                                `💲 <b>Monto:</b> $${monto} MXN\n` +
                                `💳 <b>Intento:</b> ${indiceActual + 1}/${Math.max(tarjetas.length, 1)} — <code>•••• ${sPrev.ult4 || '****'}</code>\n\n` +
                                (siguiente
                                    ? `👉 La siguiente tarjeta ya está cargada. Confirma para continuar con <code>•••• ${siguiente.ult4}</code>.`
                                    : `ℹ️ No quedan más tarjetas cargadas en esta sesión.`);
                            ...
                            if (capturaVoucher) {
                                ...
                                await ctx.reply(mensajeTelegram, { parse_mode: 'HTML', ...Markup.inlineKeyboard(filas) });
                            }
                        } else {
...
// 8. GESTOR DE INTENTO INDIVIDUAL (MISMO CONTEXTO/PÁGINA DURANTE EL CICLO)
async function ejecutarIntentoBait(ctx, id, datos, intento, nav, recursoBait = null) {
    const { numero, monto: montoIn } = datos;
    const monto = montoIn || 300;

    // Los datos de identidad se fijan una sola vez por ciclo BAIT.
    // En los reintentos técnicos se reutilizan número/correo y la misma sesión.
    const personaAuto = datos.nombre || generarNombreCompleto();
    const direccionValida = datos.direccion_valida || generarDireccionValida();
    const cpAuto = datos.cp_auto || direccionValida.cp;
    const correoDinamico = datos.correo_auto || generarCorreoUnico(personaAuto);
    const telDinamico = datos.tel_auto || generarTelefonoUnico();
    ...
    let contexto = recursoBait?.contexto || null;
    let pag = recursoBait?.pag || null;

    if (!contexto || !pag || (typeof pag.isClosed === 'function' && pag.isClosed())) {
        const creado = await crearContextoBait(nav, id);
        contexto = creado.contexto;
        pag = creado.pag;
        if (recursoBait) {
            recursoBait.contexto = contexto;
            recursoBait.pag = pag;
        }
        console.log(`🧹 [Bait Usuario ${id}] Contexto BAIT creado una sola vez para el ciclo`);
    } else {
        console.log(`♻️ [Bait Usuario ${id}] Reutilizando MISMO contexto y MISMA página para intento ${intento}`);
    }
    ...
    try {
        ...
        if (resultadoPasarela.pasarela === 'CONEKTA') {
            ...
            console.log(`🔄 [Bait Usuario ${id}] Manteniendo Chromium/contexto abiertos. El siguiente intento volverá al paquete en la MISMA página (Intento ${intento}/${MAX_RETRIES_BAIT})...`);
            return {
                exito: false,
                pagoConfirmado: false,
                pasarelaDetectada: false,
...
async function flujoBait(ctx, id, datos) {
    ...
    try {
        ...
        // Una sola identidad técnica por ciclo: los reintentos reutilizan estos datos.
        const personaCiclo = datos.nombre || generarNombreCompleto();
        const direccionCiclo = datos.direccion_valida || generarDireccionValida();
        const datosCiclo = {
            ...datos,
            nombre: personaCiclo,
            direccion_valida: direccionCiclo,
            cp_auto: datos.cp_auto || direccionCiclo.cp,
            correo_auto: datos.correo_auto || generarCorreoUnico(personaCiclo),
            tel_auto: datos.tel_auto || generarTelefonoUnico()
        };

        // Se crea como máximo un contexto/página para todos los intentos del ciclo.
        const recursoBait = { contexto: null, pag: null };

        for (let intento = 1; intento <= MAX_RETRIES_BAIT; intento++) {
            if (miId !== ejecucionesUsuario.get(id)) return;

            resultadoFinal = await ejecutarIntentoBait(ctx, id, datosCiclo, intento, nav, recursoBait);
...
async function monitorearRespuestaPagoTienda(page, tiempoClic, id) {
    ...
    if (!tiempoClic) {
        return { tipo: 'NO_ENVIADO', segundos: 0 };
    }

    const timeoutMaximo = TIMEOUT_CONFIRMACION_PAGO_MS;
    const inicio = Date.now();
    let candidato = null;
    let consecutivas = 0;
    ...
    while (Date.now() - inicio < timeoutMaximo) {
        ...
        const texto = textoPagina.replace(/\s+/g, ' ').trim();
        const tiempoRespuestaSegundos = (Date.now() - tiempoClic) / 1000;

        const esProcesando = /procesando|en proceso|validando|pendiente|espera un momento|estamos procesando tu pago|estamos procesando tu solicitud/i.test(texto);
        if (esProcesando) {
            vioProcesando = true;
            candidato = null;
            consecutivas = 0;
            if (Math.floor(tiempoRespuestaSegundos) % 10 === 0) {
                logTelcelTienda(id, `⏳ El portal sigue procesando (${tiempoRespuestaSegundos.toFixed(1)}s)`);
            }
        ...
        if (/¡?Gracias por tu compra!?|Compra de paquete|Folio Telcel|comprobante por SMS|recarga exitosa|transacci[oó]n exitosa|recarga completada/i.test(texto)) {
            tipoDetectado = 'EXITO';
        } else if (/Tu solicitud no pudo ser completada|Por favor, int[eé]ntalo m[aá]s tarde|Por el momento la forma de pago seleccionada no est[aá] disponible/i.test(texto)) {
            tipoDetectado = 'SOLICITUD_NO_COMPLETADA';
        } else if (/pago rechazado|transacci[oó]n rechazada|rechazad[oa]|declinad[oa]|fondos insuficientes|no autorizado/i.test(texto)) {
            tipoDetectado = 'RECHAZO_BANCARIO';
        } else if (/error de conexi[oó]n|no pudimos conectar|fall[oó] la conexi[oó]n|problemas de conexi[oó]n/i.test(texto)) {
            tipoDetectado = 'ERROR_CONEXION';
        }

        if (tipoDetectado) {
            if (candidato === tipoDetectado) consecutivas++;
            else {
                candidato = tipoDetectado;
                consecutivas = 1;
            }

            const requeridas = tipoDetectado === 'EXITO' ? LECTURAS_EXITO_ESTABLE : LECTURAS_ERROR_ESTABLE;
            logTelcelTienda(id, `🧭 Confirmando ${tipoDetectado}: ${consecutivas}/${requeridas}`);

            if (consecutivas >= requeridas) {
                logTelcelTienda(id, `✅ Estado terminal estable: ${tipoDetectado} en ${tiempoRespuestaSegundos.toFixed(1)}s`);
                return { tipo: tipoDetectado, segundos: tiempoRespuestaSegundos };
            }
        } else {
            candidato = null;
            consecutivas = 0;
    ...
    return {
        tipo: vioProcesando ? 'PENDIENTE_CONFIRMACION' : 'TIMEOUT',
...
async function flujoTelcelTienda(ctx, id, datos) {
    ...
    try {
        ...
        if (resultado.tipo === 'EXITO') {
            ...
            datos.paso = 'resultado_exito';
            datos.ultimoResultado = 'EXITO';
            ...
            const tecladoExitoTienda = datos.guiadoRecarga
                ? Markup.inlineKeyboard([
                    [Markup.button.callback('✅ SÍ — MISMO MÉTODO DE PAGO', 'rec_otra_mismos_datos')],
                    [Markup.button.callback('🦁 MENÚ PRINCIPAL', 'btn_reiniciar')]
                ])
                : Markup.inlineKeyboard([[Markup.button.callback('🦁 MENÚ PRINCIPAL', 'btn_reiniciar')]]);

            if (fs.existsSync(rutaConfirmacion)) {
                await ctx.replyWithPhoto(
                    { source: rutaConfirmacion },
                    { caption: textoExito, parse_mode: 'HTML', ...tecladoExitoTienda }
                ).catch(() => {});
            } else {
                await ctx.reply(textoExito, { parse_mode: 'HTML', ...tecladoExitoTienda }).catch(() => {});
            }
        } else if (resultado.tipo === 'SOLICITUD_NO_COMPLETADA' || resultado.tipo === 'RECHAZO_BANCARIO') {
            if (resultado.tipo === 'SOLICITUD_NO_COMPLETADA') {
                await ctx.replyWithHTML(
                ...
                await ctx.replyWithHTML(
                    ...
                    `👉 <b>Por favor verifique los fondos de su tarjeta o elija otra.</b>`
                ).catch(() => {});
                if (datos.guiadoRecarga) {
                    const tarjetas = obtenerTarjetasSesion(id);
                    const indiceActual = Number.isInteger(datos.tarjetaSeleccionada) ? datos.tarjetaSeleccionada : 0;
                    const siguienteIndice = indiceActual + 1;
                    const siguiente = tarjetas[siguienteIndice];
                    datos.paso = 'resultado_rechazado';
                    datos.siguienteTarjetaIndice = siguiente ? siguienteIndice : null;
                    sesiones.set(id, datos);
                    ...
                    await ctx.reply(
                        `💳 <b>Intento ${indiceActual + 1}/${Math.max(tarjetas.length, 1)} finalizado.</b>\n` +
                        (siguiente
                            ? `La siguiente tarjeta ya está cargada: <code>•••• ${siguiente.ult4}</code>.\n\n👉 Confirma para continuar.`
                            : `No quedan más tarjetas cargadas en esta sesión.`),
            ...
            await ctx.replyWithHTML(
                ...
                `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
                `El portal no mostró un estado terminal dentro del tiempo de seguridad. No se reporta éxito ni rechazo hasta tener una confirmación clara.`
            ).catch(() => {});
        }
        ...
        const s = sesiones.get(id);
        if (s && !['numero', 'resultado_exito', 'recarga_confirmar', 'recarga_seleccionar_tarjeta'].includes(s.paso)) {
            sesiones.delete(id);
        }
    }
}
...
// ==============================================================================
// 🟢 FLUJO GUIADO /recarga — TELCEL PAY + TELCEL.COM (5 PASOS)
// ==============================================================================
// Este flujo reutiliza los dos motores Telcel ya existentes:
//   - TELCEL PAY  -> flujoTelcelIndependiente()
//   - TELCEL.COM  -> flujoTelcelTienda()
//
// Montos habilitados en este flujo: $200, $300 y $500.
// Tarjetas: de 1 a 3 por sesión, en memoria; el usuario elige cuál usar.
// No se prueban tarjetas automáticamente una tras otra.

function textoResumenRecargaGuiada(s, paso, instruccion) {
    const via = s.viaRecarga || 'Pendiente';
    const paquete = s.monto ? `$${s.monto}` : 'Pendiente';
    const tarjetas = obtenerTarjetasSesion(s.idUsuario || 0);
    const tarjetasTexto = tarjetas.length ? `${tarjetas.length} cargada${tarjetas.length === 1 ? '' : 's'}` : 'Pendiente';

    return (
        `📲 <b>RECARGA TELCEL</b>\n` +
        `✅ <b>Número:</b> <code>${s.numero || 'Pendiente'}</code>\n` +
        `${s.viaRecarga ? '✅' : '🔵'} <b>Vía:</b> ${via}\n` +
        `${s.monto ? '✅' : '⚪'} <b>Paquete:</b> ${paquete}\n` +
        `${tarjetas.length ? '✅' : '⚪'} <b>Tarjetas:</b> ${tarjetasTexto}\n\n` +
        `<b>PASO ${paso}/5</b> 👇 ${instruccion}`
    );
}

async function mostrarRecargaPaso2Via(ctx, s) {
    const id = ctx.chat?.id || ctx.from?.id;
    s.idUsuario = id;
    s.paso = 'recarga_via';
    sesiones.set(id, s);
    return enviarLimpio(
        ctx,
        textoResumenRecargaGuiada(s, 2, 'Elige la vía de la recarga'),
        Markup.inlineKeyboard([
            [
                Markup.button.callback('📱 TELCEL PAY', 'rec_via_telcelpay'),
                Markup.button.callback('🟣 TELCEL.COM', 'rec_via_telcelcom')
            ],
...
async function mostrarRecargaPaso3Monto(ctx, s) {
    const id = ctx.chat?.id || ctx.from?.id;
    s.idUsuario = id;
    s.paso = 'recarga_monto';
    sesiones.set(id, s);
    return enviarLimpio(
        ctx,
        textoResumenRecargaGuiada(s, 3, 'Elige monto o paquete'),
        Markup.inlineKeyboard([
            [
                Markup.button.callback('$200', 'rec_monto_200'),
                Markup.button.callback('$300', 'rec_monto_300'),
                Markup.button.callback('$500', 'rec_monto_500')
            ],
            [Markup.button.callback('⬅️ ATRÁS', 'rec_atras_via')],
            [Markup.button.callback('❌ CANCELAR', 'btn_cancelar')]
        ])
    );
}

async function mostrarRecargaPaso4Tarjetas(ctx, s) {
    const id = ctx.chat?.id || ctx.from?.id;
    s.idUsuario = id;
    s.paso = 'recarga_tarjetas';
    sesiones.set(id, s);
    return enviarLimpio(
        ctx,
        textoResumenRecargaGuiada(s, 4, 'Envía de 1 a 3 tarjetas') +
            `\n\n💳 <b>Formato por línea:</b> <code>NUMERO|MM|AA|CVV</code>\n` +
            `👉 Puedes enviar <b>1, 2 o 3 líneas</b> en un solo mensaje.\n` +
            `🔒 Se conservan únicamente en memoria durante la sesión.`,
        Markup.inlineKeyboard([
            [Markup.button.callback('⬅️ ATRÁS', 'rec_atras_monto')],
            [Markup.button.callback('❌ CANCELAR', 'btn_cancelar')]
        ])
    );
}

async function mostrarRecargaPaso5Seleccion(ctx, s) {
    const id = ctx.chat?.id || ctx.from?.id;
    s.idUsuario = id;
    s.paso = 'recarga_seleccionar_tarjeta';
    sesiones.set(id, s);
    const tarjetas = obtenerTarjetasSesion(id);
    const filas = tarjetas.map((t, i) => [
        Markup.button.callback(`💳 TARJETA ${i + 1} •••• ${t.ult4}`, `rec_tarjeta_${i}`)
    ]);
    filas.push([Markup.button.callback('✏️ CAMBIAR TARJETAS', 'rec_reingresar_tarjetas')]);
    filas.push([Markup.button.callback('⬅️ ATRÁS', 'rec_atras_monto')]);
    filas.push([Markup.button.callback('❌ CANCELAR', 'btn_cancelar')]);

    return enviarLimpio(
        ctx,
        textoResumenRecargaGuiada(s, 5, 'Elige qué tarjeta usar') +
            `\n\n${tarjetas.map((t, i) => `${i + 1}️⃣ <code>•••• ${t.ult4}</code>`).join('\n')}`,
        Markup.inlineKeyboard(filas)
    );
}

async function mostrarRecargaConfirmacion(ctx, s) {
    const id = ctx.chat?.id || ctx.from?.id;
    s.idUsuario = id;
    s.paso = 'recarga_confirmar';
    sesiones.set(id, s);
    return enviarLimpio(
        ctx,
        `✅ <b>CONFIRMA LA RECARGA</b>\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
        `📱 <b>Número:</b> <code>${s.numero}</code>\n` +
        `🛣️ <b>Vía:</b> ${s.viaRecarga}\n` +
                `💲 <b>Monto:</b> $${s.monto} MXN\n` +
        `💳 <b>Tarjeta:</b> <code>•••• ${s.ult4 || '****'}</code>\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
        `👉 Confirma para iniciar <b>un solo intento</b> de recarga.`,
        Markup.inlineKeyboard([
            [Markup.button.callback('✅ CONFIRMAR RECARGA', 'rec_confirmar_pago')],
            [Markup.button.callback('💳 ELEGIR OTRA TARJETA', 'rec_volver_tarjetas')],
            [Markup.button.callback('❌ CANCELAR', 'btn_cancelar')]
        ])
    );
}

function parsearHastaTresTarjetas(texto) {
    ...
    if (lineas.length < 1 || lineas.length > 3) {
        return { valido: false, error: 'Debes enviar entre 1 y 3 tarjetas, una por línea.' };
    }

    const tarjetas = [];
    for (let i = 0; i < lineas.length; i++) {
        const r = parsearDatosTarjeta(lineas[i]);
        if (!r.valido) {
            return { valido: false, error: `La tarjeta ${i + 1} no tiene un formato válido.` };
        }
        tarjetas.push(r);
    }
    return { valido: true, tarjetas };
}

async function prepararOtraRecargaMismosDatos(ctx, s) {
    const id = ctx.chat?.id || ctx.from?.id;
    if (!s || !s.guiadoRecarga || !s.monto || !(s.tarjeta || s.cc)) {
        return ctx.reply('⚠️ No hay datos completos de una recarga guiada anterior.');
    }

    // Reutiliza SOLO los datos de la recarga que ya terminó con éxito:
    // vía, monto, datos auxiliares y la misma tarjeta seleccionada.
    // El único dato que se vuelve a pedir es la nueva línea Telcel.
    s.numero = null;
    s.paso = 'recarga_numero';
    s.reusarMismosDatos = true;
    s.intentosTarjeta = 0;
    sesiones.set(id, s);

    if (s.flujoModuloDirecto) {
        return mostrarModuloPaso1Numero(ctx, s);
    }

    return ctx.replyWithHTML(
        `🔄 <b>NUEVA RECARGA — MISMO MÉTODO DE PAGO</b>\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
        `💳 Método conservado: <code>•••• ${s.ult4 || '****'}</code>\n` +
        `💲 Monto: <b>$${s.monto} MXN</b>\n` +
        `🧾 Se reutilizarán los datos auxiliares de la recarga anterior (por ejemplo nombre, correo y CP, según la vía).\n\n` +
        `📱 Escribe únicamente el <b>nuevo número de 10 dígitos</b>.`
    );
}


// ==============================================================================
// NOTA DE SEGURIDAD DE PAGO: las tarjetas cargadas NO se prueban en secuencia automática.
// Ante un rechazo, el usuario debe elegir explícitamente el siguiente método.
// Tras un ÉXITO sí puede reutilizarse el MISMO método para una nueva línea.

// 🎨 FLUJO VISUAL UNIFICADO — TELCEL PAY / TELCEL.COM (5 PASOS)
// ==============================================================================
// Entrada directa desde los botones del menú principal:
//   1/5 Número
//   2/5 Monto
//   3/5 Tarjetas (1 a 3)
//   4/5 Elegir tarjeta
//   5/5 Confirmar recarga
//
// La vía (TELCEL PAY o TELCEL.COM) ya viene elegida desde el menú y se muestra
// siempre en el encabezado. Ambos módulos comparten esta misma interfaz.

function iconoViaRecarga(via) {
    return String(via || '').toUpperCase().includes('TELCEL.COM') ? '🟣' : '🟢';
}

function barraProgresoRecarga(paso, total = 5) {
    const p = Math.max(1, Math.min(total, Number(paso) || 1));
    return '🟩'.repeat(p) + '⬜'.repeat(total - p);
}

function textoResumenModuloTelcel(s, paso, instruccion) {
    const icono = iconoViaRecarga(s.viaRecarga);
    const numero = s.numero || 'Pendiente';
    const monto = s.monto ? `$${s.monto} MXN` : 'Pendiente';
    const tarjetas = obtenerTarjetasSesion(s.idUsuario || 0);
    const tarjetasTexto = tarjetas.length
        ? `${tarjetas.length} cargada${tarjetas.length === 1 ? '' : 's'}`
        : 'Pendiente';
    const tarjetaActiva = s.ult4 ? `•••• ${s.ult4}` : 'Pendiente';

    return (
        `${icono} <b>RECARGA ${s.viaRecarga || 'TELCEL'}</b>\n` +
        `${barraProgresoRecarga(paso)}  <b>${paso}/5</b>\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
        `${s.numero ? '✅' : '🔵'} <b>Número:</b> <code>${numero}</code>\n` +
        `✅ <b>Vía:</b> ${s.viaRecarga || 'Pendiente'}\n` +
        `${s.monto ? '✅' : '⚪'} <b>Monto:</b> ${monto}\n` +
        `${tarjetas.length ? '✅' : '⚪'} <b>Tarjetas:</b> ${tarjetasTexto}\n` +
        `${s.ult4 ? '✅' : '⚪'} <b>Seleccionada:</b> <code>${tarjetaActiva}</code>\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
        `👉 <b>${instruccion}</b>`
    );
}

async function iniciarRecargaGuiadaModulo(ctx, via) {
    const id = ctx.chat?.id || ctx.from?.id;

    await cerrarSesionNavegador(id).catch(() => {});
    if (cerrarNavegadorTienda) await cerrarNavegadorTienda(id).catch(() => {});
    tarjetasSesionUsuario.delete(id);

    const esTienda = String(via).toUpperCase().includes('TELCEL.COM');
    const s = {
        idUsuario: id,
        guiadoRecarga: true,
        flujoModuloDirecto: true,
        numero: null,
        viaRecarga: esTienda ? 'TELCEL.COM' : 'TELCEL PAY',
        tipo: esTienda ? 'Telcel.com' : 'Telcel',
        modo: esTienda ? 'telcel_tienda' : 'telcel',
        tipoRecarga: 'SIN LÍMITE',
        monto: null,
...
async function mostrarModuloPaso1Numero(ctx, s) {
    const id = ctx.chat?.id || ctx.from?.id;
    s.idUsuario = id;
    s.paso = 'recarga_numero';
    sesiones.set(id, s);
    return enviarLimpio(
        ctx,
        textoResumenModuloTelcel(s, 1, 'Escribe el número celular de 10 dígitos') +
            `\n\n📱 Ejemplo: <code>5512345678</code>`,
        Markup.inlineKeyboard([
            [Markup.button.callback('❌ CANCELAR', 'btn_cancelar')]
        ])
    );
...
async function mostrarModuloPaso2Monto(ctx, s) {
    const id = ctx.chat?.id || ctx.from?.id;
    s.idUsuario = id;
    s.paso = 'recarga_monto';
    sesiones.set(id, s);
    return enviarLimpio(
        ctx,
        textoResumenModuloTelcel(s, 2, 'Elige el monto de la recarga'),
        Markup.inlineKeyboard([
            [
                Markup.button.callback('💲 $200', 'rec_monto_200'),
                Markup.button.callback('💲 $300', 'rec_monto_300'),
            ...
            [Markup.button.callback('⬅️ ATRÁS', 'rec_mod_atras_numero')],
            [Markup.button.callback('❌ CANCELAR', 'btn_cancelar')]
        ])
    );
}

async function mostrarModuloPaso3Tarjetas(ctx, s) {
    const id = ctx.chat?.id || ctx.from?.id;
    s.idUsuario = id;
    s.paso = 'recarga_tarjetas';
    sesiones.set(id, s);
    return enviarLimpio(
        ctx,
        textoResumenModuloTelcel(s, 3, 'Envía de 1 a 3 tarjetas en un solo mensaje') +
            `\n\n💳 <b>Una tarjeta por línea:</b>\n` +
            `<code>NUMERO|MM|AA|CVV</code>\n\n` +
            `🔒 El mensaje con los datos se elimina del chat cuando es leído.`,
        Markup.inlineKeyboard([
            [Markup.button.callback('⬅️ ATRÁS', 'rec_mod_atras_monto')],
            [Markup.button.callback('❌ CANCELAR', 'btn_cancelar')]
        ])
    );
}

async function mostrarModuloPaso4Seleccion(ctx, s) {
    const id = ctx.chat?.id || ctx.from?.id;
    s.idUsuario = id;
    s.paso = 'recarga_seleccionar_tarjeta';
    sesiones.set(id, s);
    ...
    filas.push([Markup.button.callback('✏️ CAMBIAR TARJETAS', 'rec_reingresar_tarjetas')]);
    filas.push([Markup.button.callback('⬅️ ATRÁS', 'rec_mod_atras_tarjetas')]);
    filas.push([Markup.button.callback('❌ CANCELAR', 'btn_cancelar')]);

    return enviarLimpio(
        ctx,
        textoResumenModuloTelcel(s, 4, 'Elige la tarjeta que quieres usar') +
            `\n\n${tarjetas.map((t, i) => `${i + 1}️⃣ <code>•••• ${t.ult4}</code>`).join('\n')}`,
        Markup.inlineKeyboard(filas)
    );
}

async function mostrarModuloPaso5Confirmacion(ctx, s) {
    const id = ctx.chat?.id || ctx.from?.id;
    s.idUsuario = id;
    s.paso = 'recarga_confirmar';
    sesiones.set(id, s);
    return enviarLimpio(
        ctx,
        textoResumenModuloTelcel(s, 5, 'Revisa los datos y confirma la recarga') +
            `\n\n⚠️ Se realizará <b>un solo intento</b> con la tarjeta seleccionada. Si falla y hay otra tarjeta cargada, el bot te mostrará un botón para confirmar el siguiente intento sin volver a capturar datos.`,
        Markup.inlineKeyboard([
            [Markup.button.callback('✅ CONFIRMAR RECARGA', 'rec_confirmar_pago')],
            [Markup.button.callback('💳 CAMBIAR TARJETA', 'rec_volver_tarjetas')],
            [Markup.button.callback('⬅️ ATRÁS', 'rec_mod_atras_seleccion')],
            [Markup.button.callback('❌ CANCELAR', 'btn_cancelar')]
        ])
    );
}
...
// 📲 COMANDO GUIADO: /recarga 5512345678
bot.command('recarga', async ctx => {
    ...
    if (!/^\d{10}$/.test(numero)) {
        return enviarLimpio(
            ctx,
            `📲 <b>RECARGA TELCEL — PASO 1/5</b>\n\n` +
            `Escribe el comando con un número de 10 dígitos:\n` +
            `<code>/recarga 5512345678</code>`,
    ...
    sesiones.set(id, s);
    return mostrarRecargaPaso2Via(ctx, s);
});

bot.action('rec_via_telcelpay', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    const id = ctx.chat?.id || ctx.from?.id;
    const s = sesiones.get(id);
    if (!s?.guiadoRecarga) return mostrarMenuInicio(ctx);
    s.viaRecarga = 'TELCEL PAY';
    s.tipo = 'Telcel';
    s.modo = 'telcel';
    s.tipoRecarga = 'SIN LÍMITE'; // Confirmación interna; no requiere un paso separado.
    return mostrarRecargaPaso3Monto(ctx, s);
});

bot.action('rec_via_telcelcom', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    const id = ctx.chat?.id || ctx.from?.id;
    const s = sesiones.get(id);
    if (!s?.guiadoRecarga) return mostrarMenuInicio(ctx);
    s.viaRecarga = 'TELCEL.COM';
    s.tipo = 'Telcel.com';
...
bot.action(/^rec_monto_(200|300|500)$/, async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    const id = ctx.chat?.id || ctx.from?.id;
    const s = sesiones.get(id);
    if (!s?.guiadoRecarga) return mostrarMenuInicio(ctx);
    s.monto = Number(ctx.match[1]);
    if (s.flujoModuloDirecto) return mostrarModuloPaso3Tarjetas(ctx, s);
    return mostrarRecargaPaso4Tarjetas(ctx, s);
});

bot.action(/^rec_tarjeta_(\d+)$/, async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    const id = ctx.chat?.id || ctx.from?.id;
    const s = sesiones.get(id);
    if (!s?.guiadoRecarga) return mostrarMenuInicio(ctx);
    ...
    const tarjetas = obtenerTarjetasSesion(id);
    if (!tarjetas[indice]) return mostrarRecargaPaso5Seleccion(ctx, s);
    aplicarTarjetaASesion(s, tarjetas[indice]);
    s.tarjetaSeleccionada = indice;
    if (s.flujoModuloDirecto) return mostrarModuloPaso5Confirmacion(ctx, s);
    return mostrarRecargaConfirmacion(ctx, s);
});

bot.action('rec_confirmar_pago', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    const id = ctx.chat?.id || ctx.from?.id;
    const s = sesiones.get(id);
    if (!s?.guiadoRecarga || !(s.tarjeta || s.cc) || !s.monto || !s.numero) {
        return ctx.reply('⚠️ Faltan datos para confirmar la recarga.');
    }
    s.paso = 'ejecutando';
    sesiones.set(id, s);
...
bot.action('rec_confirmar_siguiente_tarjeta', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    const id = ctx.chat?.id || ctx.from?.id;
    const s = sesiones.get(id);
    if (!s?.guiadoRecarga) return mostrarMenuInicio(ctx);

    const indice = Number(s.siguienteTarjetaIndice);
    const tarjetas = obtenerTarjetasSesion(id);
    const tarjeta = Number.isInteger(indice) ? tarjetas[indice] : null;

    if (!tarjeta) {
        return ctx.reply('⚠️ No hay una siguiente tarjeta disponible. Inicia una recarga nueva.');
    }

    aplicarTarjetaASesion(s, tarjeta);
    s.tarjetaSeleccionada = indice;
    s.siguienteTarjetaIndice = null;
    s.paso = 'recarga_confirmar';
    sesiones.set(id, s);

    // Se conserva número, vía, monto y datos auxiliares.
    // El usuario confirma explícitamente antes del siguiente intento.
    return mostrarModuloPaso5Confirmacion(ctx, s);
});

bot.action('rec_volver_tarjetas', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    const id = ctx.chat?.id || ctx.from?.id;
    const s = sesiones.get(id);
    if (!s?.guiadoRecarga) return mostrarMenuInicio(ctx);
    if (s.flujoModuloDirecto) return mostrarModuloPaso4Seleccion(ctx, s);
    return mostrarRecargaPaso5Seleccion(ctx, s);
});

bot.action('rec_reingresar_tarjetas', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    const id = ctx.chat?.id || ctx.from?.id;
    const s = sesiones.get(id);
    if (!s?.guiadoRecarga) return mostrarMenuInicio(ctx);
...
bot.action('rec_atras_via', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    const id = ctx.chat?.id || ctx.from?.id;
    const s = sesiones.get(id);
    if (!s?.guiadoRecarga) return mostrarMenuInicio(ctx);
...
bot.action('rec_atras_monto', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    const id = ctx.chat?.id || ctx.from?.id;
    const s = sesiones.get(id);
    if (!s?.guiadoRecarga) return mostrarMenuInicio(ctx);
...
bot.action('rec_mod_atras_numero', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    const id = ctx.chat?.id || ctx.from?.id;
    const s = sesiones.get(id);
    if (!s?.flujoModuloDirecto) return mostrarMenuInicio(ctx);
    s.numero = null;
    s.monto = null;
    return mostrarModuloPaso1Numero(ctx, s);
});

bot.action('rec_mod_atras_monto', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    const id = ctx.chat?.id || ctx.from?.id;
    const s = sesiones.get(id);
    if (!s?.flujoModuloDirecto) return mostrarMenuInicio(ctx);
    s.monto = null;
    return mostrarModuloPaso2Monto(ctx, s);
});

bot.action('rec_mod_atras_tarjetas', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    const id = ctx.chat?.id || ctx.from?.id;
    const s = sesiones.get(id);
    if (!s?.flujoModuloDirecto) return mostrarMenuInicio(ctx);
    ...
    s.cvv = null;
    s.ult4 = null;
    return mostrarModuloPaso3Tarjetas(ctx, s);
});

bot.action('rec_mod_atras_seleccion', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    const id = ctx.chat?.id || ctx.from?.id;
    const s = sesiones.get(id);
    if (!s?.flujoModuloDirecto) return mostrarMenuInicio(ctx);
    ...
    s.cvv = null;
    s.ult4 = null;
    s.tarjetaSeleccionada = null;
    return mostrarModuloPaso4Seleccion(ctx, s);
});

bot.action('rec_otra_mismos_datos', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    const id = ctx.chat?.id || ctx.from?.id;
    const s = sesiones.get(id);
    return prepararOtraRecargaMismosDatos(ctx, s);
});
...
bot.action('btn_telcel', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    return iniciarRecargaGuiadaModulo(ctx, 'TELCEL PAY');
});

bot.action('btn_telcel_tienda', async ctx => {
    await ctx.answerCbQuery().catch(() => {});
    return iniciarRecargaGuiadaModulo(ctx, 'TELCEL.COM');
});
...
bot.on('text', async (ctx, next) => {
    ...
    // 🎨 TELCEL PAY / TELCEL.COM — PASO 1/5: CAPTURA DE NÚMERO
    if (s?.guiadoRecarga && s.flujoModuloDirecto && s.paso === 'recarga_numero') {
        const numero = txt.replace(/\D/g, '');
        if (!/^\d{10}$/.test(numero)) {
            return enviarLimpio(
                ctx,
                textoResumenModuloTelcel(s, 1, 'Escribe un número válido de 10 dígitos') +
                    `\n\n⚠️ Ejemplo: <code>5512345678</code>`,
                Markup.inlineKeyboard([[Markup.button.callback('❌ CANCELAR', 'btn_cancelar')]])
            );
        }
        ...
        // Si viene de una recarga exitosa y eligió “MISMO PAGO”,
        // conserva vía, monto, datos auxiliares y la tarjeta que ya funcionó.
        // Sólo cambia el número y pasa a la confirmación final.
        if (s.reusarMismosDatos && s.monto && (s.tarjeta || s.cc)) {
            s.reusarMismosDatos = false;
            s.paso = 'recarga_confirmar';
            sesiones.set(id, s);
            return mostrarModuloPaso5Confirmacion(ctx, s);
        }
    ...
    // 🟢 FLUJO GUIADO /recarga — CAPTURA DE 1 A 3 TARJETAS EN UN SOLO MENSAJE
    if (s?.guiadoRecarga && s.paso === 'recarga_tarjetas') {
        const lote = parsearHastaTresTarjetas(txt);
        if (!lote.valido) {
            return enviarLimpio(
                ctx,
                `⚠️ <b>NO PUDE LEER LAS TARJETAS</b>\n\n` +
                `${lote.error}\n\n` +
                `Formato por línea: <code>NUMERO|MM|AA|CVV</code>\n` +
                `Envía entre <b>1 y 3 líneas</b>.`,
                Markup.inlineKeyboard([
                    [Markup.button.callback('⬅️ ATRÁS', 'rec_atras_monto')],
                    [Markup.button.callback('❌ CANCELAR', 'btn_cancelar')]
                ])
        ...
        // Borra de inmediato el mensaje que contenía datos sensibles, si Telegram lo permite.
        await ctx.deleteMessage().catch(() => {});
        tarjetasSesionUsuario.delete(id);

        for (const t of lote.tarjetas) {
            guardarTarjetaSesion(id, t);
        }

        // Datos auxiliares necesarios por los motores Telcel existentes.
        ...
        s.cp_auto = dirValida.cp;
        s.tel_auto = generarTelefonoUnico();
        s.correo_auto = generarCorreoUnico(datosPersona);
        s.tarjeta = null;
        s.cc = null;
        s.cvv = null;
        s.ult4 = null;
