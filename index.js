    ...
    try {
        await ejecutarConReintento(async (intento) => {
            ...
            try {
                ...
                // 4. ESPERA ACTIVA ROBUSTA (MÁXIMO 180s, sin cerrar mientras siga PROCESANDO)
                ultimaEtapa = "Analizando respuesta y comprobante";
                logTelcel(id, `⏳ Esperando confirmación/respuesta definitiva del portal (máx 180s)...`);
                const inicioEspera = Date.now();
                const TIEMPO_MAXIMO_ESPERA_TELCEL_MS = 180000; // 3 min: margen extra para servidor / portal lento
                let textoFinalPagina = "";
                let clasificacionFinal = null;
                let ultimoEstadoLog = null;
                let candidatoTerminal = null;
                let candidatoTerminalVeces = 0;
                const TERMINALES_TELCEL = new Set(['EXITO', 'RECHAZO_BANCARIO', 'BLOQUEO_TELCEL', 'ERROR_TELCEL']);
                ...
                while (Date.now() - inicioEspera < TIEMPO_MAXIMO_ESPERA_TELCEL_MS) {
                    ...
                    // Un estado terminal debe verse estable en varias lecturas antes de cerrar el proceso.
                    // Esto evita tomar como definitivo un modal/texto transitorio mientras el portal sigue cambiando.
                    if (TERMINALES_TELCEL.has(clasificacionFinal.estado)) {
                        if (candidatoTerminal === clasificacionFinal.estado) {
                            candidatoTerminalVeces += 1;
                        } else {
                            candidatoTerminal = clasificacionFinal.estado;
                            candidatoTerminalVeces = 1;
                        }

                        const lecturasNecesarias = clasificacionFinal.estado === 'EXITO' ? 2 : 3;
                        if (candidatoTerminalVeces >= lecturasNecesarias) {
                            logTelcel(id, `✅ Estado terminal estable confirmado: ${clasificacionFinal.estado} (${candidatoTerminalVeces} lecturas)`);
                            break;
                        }
                    } else {
                        candidatoTerminal = null;
                        candidatoTerminalVeces = 0;
                    }
...
` +
                        ...
                        if (sPrev.guiadoRecarga) {
                            sPrev.paso = 'recarga_seleccionar_tarjeta';
                            sesiones.set(id, sPrev);
                            const tarjetas = obtenerTarjetasSesion(id);
                            const filas = tarjetas.map((t, i) => [
                                Markup.button.callback(`💳 USAR TARJETA ${i + 1} •••• ${t.ult4}`, `rec_tarjeta_${i}`)
                            ]);
                            ...
                            mensajeTelegram =
                                ...
                                `📱 <b>Línea:</b> <code>${numero}</code>\n` +
                                `💲 <b>Monto:</b> $${monto} MXN\n\n` +
                                `💳 La tarjeta seleccionada fue rechazada.\n` +
                                `👉 Si quieres hacer otro intento, <b>elige manualmente</b> una de las tarjetas ya cargadas.`;
                        ...
                        mensajeTelegram = 
                            ...
                            `ℹ️ No se marcará como error mientras no exista una respuesta terminal clara.
` +
                            `👉 <b>Toca /start solo si deseas abandonar esta operación.</b>`;
...
async function monitorearRespuestaPagoTienda(page, tiempoClic, id) {
    ...
    const timeoutMaximo = 180000; // 3 min: margen extra antes de declarar pendiente
    const inicio = Date.now();
    let ultimoEstado = 'ESPERANDO';
    let candidatoTerminal = null;
    let candidatoVeces = 0;
    ...
    while (Date.now() - inicio < timeoutMaximo) {
        ...
        // Mientras el portal diga que está procesando, jamás cerrar ni clasificar como error por tiempo corto.
        const estaProcesando = /(procesando|en proceso|validando|espera un momento|estamos procesando|transacci[oó]n pendiente|pago pendiente)/i.test(textoPagina);
        if (estaProcesando && ultimoEstado !== 'PROCESANDO') {
            ultimoEstado = 'PROCESANDO';
            logTelcelTienda(id, `⏳ El portal sigue procesando el pago (${tiempoRespuestaSegundos.toFixed(1)}s)...`);
        }
        ...
        if (/¡?Gracias por tu compra!?|Compra de paquete|Folio Telcel|comprobante por SMS|recarga exitosa|transacci[oó]n exitosa|recarga completada/i.test(textoPagina)) {
            ...
            detectado = 'FORMA_PAGO_NO_DISPONIBLE';
            extra.esRapido = tiempoRespuestaSegundos < 5.0;
        } else if (/tu solicitud no pudo ser completada|por favor,? int[eé]ntalo m[aá]s tarde|no se pudo completar (la )?solicitud|operaci[oó]n no pudo ser completada/i.test(textoPagina)) {
            detectado = 'SOLICITUD_NO_COMPLETADA';
        } else if (/error de conexi[oó]n|no pudimos conectar|fall[oó] la conexi[oó]n|problemas de conexi[oó]n/i.test(textoPagina)) {
            detectado = 'ERROR_CONEXION';
        }

        // Confirmar estabilidad del resultado antes de cerrar; evita capturas de mensajes transitorios.
        if (detectado) {
            if (candidatoTerminal === detectado) candidatoVeces += 1;
            else {
                candidatoTerminal = detectado;
                candidatoVeces = 1;
            }

            const lecturasNecesarias = detectado === 'EXITO' ? 2 : 3;
            if (candidatoVeces >= lecturasNecesarias) {
                logTelcelTienda(id, `✅ Resultado terminal estable: ${detectado} en ${tiempoRespuestaSegundos.toFixed(1)}s`);
                return {
                    tipo: detectado,
...
async function flujoTelcelTienda(ctx, id, datos) {
    ...
    try {
        ...
        if (resultado.tipo === 'EXITO') {
            ...
            if (resultado.esRapido) {
                ...
                if (datos.guiadoRecarga) {
                    datos.paso = 'recarga_seleccionar_tarjeta';
                    sesiones.set(id, datos);
                    await mostrarRecargaPaso5Seleccion(ctx, datos).catch(() => {});
                }
            }
        } else if (resultado.tipo === 'SOLICITUD_NO_COMPLETADA') {
            await ctx.replyWithHTML(
                `⚠️ <b>TELCEL.COM — SOLICITUD NO COMPLETADA</b>\n` +
                `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
                `El portal confirmó que esta solicitud no pudo completarse.\n\n` +
                `ℹ️ Este resultado se trata como terminal; no se confunde con el estado "procesando".\n` +
                `👉 Puedes volver al menú o iniciar un nuevo intento de forma explícita.`
            ).catch(() => {});
        ...
        const s = sesiones.get(id);
        if (s && !['numero', 'resultado_exito', 'resultado_pendiente', 'recarga_confirmar', 'recarga_seleccionar_tarjeta'].includes(s.paso)) {
            sesiones.delete(id);
        }
    }
}
...
async function mostrarModuloPaso5Confirmacion(ctx, s) {
    ...
    return enviarLimpio(
        ...
        textoResumenModuloTelcel(s, 5, 'Revisa los datos y confirma la recarga') +
            `\n\n⚠️ Se realizará <b>un solo intento</b> con la tarjeta seleccionada.`,
        Markup.inlineKeyboard([
            [Markup.button.callback('✅ CONFIRMAR RECARGA', 'rec_confirmar_pago')],
