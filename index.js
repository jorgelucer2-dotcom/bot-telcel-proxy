...
// ==============================================================================
// 🧪 TELCEL PRUEBA — RECARGA AMIGO (BLOQUE INDEPENDIENTE)
// URL: https://www.telcel.com/personas/amigo/recargas/recarga-amigo
//
// OBJETIVO DE PRUEBA:
// - Mantener intacto el flujo TELCEL.COM anterior.
// - Reutilizar la misma captura de datos del bot y la misma lógica de llenado.
// - Probar selectores confirmados de la nueva página.
// - Dejar el checkout listo y detenerse ANTES de confirmar el pago final.
// ==============================================================================
...
async function cerrarNavegadorTelcelPrueba(id) {
    if (!id) {
        const entries = Array.from(navegadoresTelcelPrueba.entries());
        navegadoresTelcelPrueba.clear();
        for (const [uid, nav] of entries) {
            try {
                for (const c of nav.contexts()) {
                    for (const pg of c.pages()) await pg.close().catch(() => {});
                    await c.close().catch(() => {});
                }
                await nav.close().catch(() => {});
                liberarSlotChromiumLocal(nav, `TELCEL_PRUEBA:${uid}`);
            } catch (_) { liberarSlotChromiumLocal(nav, `TELCEL_PRUEBA:${uid}`); }
        }
        return;
    }
    ...
    } catch (_) { liberarSlotChromiumLocal(nav, `TELCEL_PRUEBA:${id}`); }
}

async function crearNavegadorTelcelPrueba(id) {
    await cerrarNavegadorTelcelPrueba(id);
    const usarBrightData = !USE_LOCAL_CHROMIUM && Boolean(
        BRIGHTDATA_BROWSER_WS && (BRIGHTDATA_BROWSER_WS.startsWith('wss://') || BRIGHTDATA_BROWSER_WS.startsWith('ws://'))
    );
...
async function seleccionarPaqueteTelcelPrueba(page, monto, id) {
    const textoMonto = `Recarga ${monto}`;
    logTelcelPrueba(id, `📦 Buscando tarjeta exacta: ${textoMonto}`);

    const candidatos = [
        page.locator('div.packages-grid__item').filter({ hasText: textoMonto }),
        page.locator('div[class*="packages-grid__item"]').filter({ hasText: textoMonto }),
        page.locator('div[class*="CardPackages"]').filter({ hasText: textoMonto })
    ];

    let tarjeta = null;
    for (const c of candidatos) {
        const loc = c.first();
        if (await loc.isVisible({ timeout: 2500 }).catch(() => false)) { tarjeta = loc; break; }
    }
    if (!tarjeta) throw new Error(`TELCEL_PRUEBA_PAQUETE_${monto}_NO_VISIBLE`);

    const boton = tarjeta.getByRole('button', { name: 'Lo quiero', exact: true }).first();
    await boton.waitFor({ state: 'visible', timeout: 8000 });
    await boton.click();
    logTelcelPrueba(id, `✅ Paquete ${textoMonto} seleccionado`);
}

async function llenarNumeroTelcelPrueba(page, numero, id) {
    const mdn = page.locator('input[name="mdn"]').first();
    const confirmMdn = page.locator('input[name="confirmMdn"]').first();
    await mdn.waitFor({ state: 'visible', timeout: 12000 });
    await mdn.fill(numero);
    await mdn.dispatchEvent('input').catch(() => {});
    await mdn.dispatchEvent('change').catch(() => {});
    await confirmMdn.fill(numero);
    await confirmMdn.dispatchEvent('input').catch(() => {});
    await confirmMdn.dispatchEvent('change').catch(() => {});
    await confirmMdn.blur().catch(() => {});

    const continuar = page.locator('button[type="submit"][aria-label="Continuar a confirmación de datos"]').first();
    await continuar.waitFor({ state: 'visible', timeout: 8000 });
    const t0 = Date.now();
    while (Date.now() - t0 < 10000) {
        if (await continuar.isEnabled().catch(() => false)) break;
        await page.waitForTimeout(250);
    }
    if (!(await continuar.isEnabled().catch(() => false))) throw new Error('TELCEL_PRUEBA_CONTINUAR_NO_HABILITADO');
    await continuar.click();
    logTelcelPrueba(id, '✅ Número y confirmación validados; Continuar ejecutado');
}

async function seleccionarMetodoTarjetaTelcelPrueba(page, id) {
    const t0 = Date.now();
    while (Date.now() - t0 < 15000) {
        for (const frame of page.frames()) {
            const radio = frame.locator('input[type="radio"][name="paymentType"][value="CARD"]').first();
            if (await radio.count().catch(() => 0)) {
                await radio.check({ force: true }).catch(async () => radio.click({ force: true }));
                logTelcelPrueba(id, '✅ Método CARD seleccionado');
                return frame;
            }
        }
        await page.waitForTimeout(300);
    }
    throw new Error('TELCEL_PRUEBA_CARD_NO_VISIBLE');
}

async function prepararFormularioPagoTelcelPrueba(page, datos, id) {
    const { tarjeta, mes, anio, cvv, cp_auto, correo_auto } = datos;
    const cleanTarjeta = String(tarjeta || '').replace(/\D/g, '');
    const cleanCvv = String(cvv || '').replace(/\D/g, '');
    const exp = `${String(mes).padStart(2, '0')}/${String(anio).slice(-2)}`;
    const cp = String(cp_auto || (datos.direccion_valida && datos.direccion_valida.cp) || '06000');
    const correo = correo_auto || 'recargas.telcel.auto@gmail.com';

    const frames = page.frames();
    let frame = frames.find(f => f.url().includes('paymentservice.telcel.com')) || page.mainFrame();
    for (const f of frames) {
        if (await f.locator('input[name="cardNumber"]').count().catch(() => 0)) { frame = f; break; }
    }

    const obtenerInput = async (sels) => {
        for (const sel of sels) {
            const loc = frame.locator(sel).first();
            if (await loc.isVisible({ timeout: 350 }).catch(() => false)) return loc;
        }
        return null;
    };
    const escribir = async (loc, valor, etiqueta, delay=20) => {
        if (!loc) throw new Error(`TELCEL_PRUEBA_CAMPO_${etiqueta}_NO_VISIBLE`);
        await loc.scrollIntoViewIfNeeded().catch(() => {});
        await loc.click({ force: true }).catch(() => {});
        await loc.fill('').catch(() => {});
        await loc.pressSequentially(String(valor), { delay }).catch(async () => loc.fill(String(valor)));
        await loc.dispatchEvent('input').catch(() => {});
        await loc.dispatchEvent('change').catch(() => {});
        await loc.blur().catch(() => {});
    };

    // Selector de Número de tarjeta confirmado por inspección: name="cardNumber"
    await escribir(await obtenerInput([
        'input[name="cardNumber"]', 'input[autocomplete="cc-number"]',
        'input[placeholder="0000 0000 0000 0000"]'
    ]), cleanTarjeta, 'TARJETA', 25);

    await escribir(await obtenerInput([
        'input[autocomplete="cc-exp"]', 'input[name*="exp" i]',
        'input[placeholder*="MM/AA"]', 'input[placeholder*="MM / AA"]'
    ]), exp, 'VENCIMIENTO', 25);
    ...
    await escribir(await obtenerInput([
        'input[autocomplete="postal-code"]', 'input[name*="postal" i]', 'input[name="cp"]',
        'input[placeholder="00000"]'
    ]), cp, 'CP', 20);
    ...
    // Reutiliza el detector de términos ya probado en TELCEL.COM.
    await aceptarTerminosTienda(page, id).catch(() => {});

    let btnPagar = null;
    for (const f of page.frames()) {
        for (const sel of ['button.btn.btn-primary:has-text("Pagar")','button[type="submit"]:has-text("Pagar")','button:has-text("Pagar")']) {
            const b = f.locator(sel).first();
            if (await b.isVisible({ timeout: 500 }).catch(() => false)) { btnPagar = b; break; }
        }
        if (btnPagar) break;
    }
    if (!btnPagar) throw new Error('TELCEL_PRUEBA_BOTON_PAGAR_NO_VISIBLE');

    const t0 = Date.now();
    while (Date.now() - t0 < 10000) {
        if (await btnPagar.isEnabled().catch(() => false)) break;
        await page.waitForTimeout(300);
    }
    if (!(await btnPagar.isEnabled().catch(() => false))) throw new Error('TELCEL_PRUEBA_BOTON_PAGAR_NO_HABILITADO');

    logTelcelPrueba(id, '✅ Formulario completo y botón Pagar habilitado');
    return { listo: true, btnPagar };
}
...
async function flujoTelcelPrueba(ctx, id, datos) {
    ...
    try {
        await ctx.replyWithHTML(
            ...
            `📱 <b>Línea:</b> <code>${numero}</code>\n` +
            `💲 <b>Monto:</b> $${monto} MXN\n` +
            `💳 <b>Método:</b> <code>**** ${(tarjeta || '').slice(-4)}</code>\n\n` +
            `⏳ Probando el nuevo flujo de Recarga Amigo...`
        ).catch(() => {});

        const sesion = await crearNavegadorTelcelPrueba(id);
        page = sesion.page;
        await page.goto(URL_TELCEL_PRUEBA, { waitUntil: 'commit', timeout: 30000 });
        await page.waitForLoadState('domcontentloaded', { timeout: 12000 }).catch(() => {});
        await aceptarCookiesTienda(page).catch(() => {});

        await seleccionarPaqueteTelcelPrueba(page, monto, id);
        await llenarNumeroTelcelPrueba(page, numero, id);
        await seleccionarMetodoTarjetaTelcelPrueba(page, id);
        await prepararFormularioPagoTelcelPrueba(page, datos, id);

        const captura = await page.screenshot({ fullPage: false }).catch(() => null);
        const caption =
            `🧪 <b>TELCEL PRUEBA — CHECKOUT LISTO</b>\n` +
            `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
            `✅ Paquete $${monto} seleccionado\n` +
            `✅ Número validado\n` +
            `✅ Método CARD seleccionado\n` +
            `✅ Formulario de pago completado\n` +
            `✅ Botón Pagar habilitado\n\n` +
            `🛑 <b>Prueba detenida antes de confirmar el pago final.</b>\n` +
            `Esto permite validar el nuevo bloque sin afectar TELCEL.COM anterior.`;

        if (captura) await ctx.replyWithPhoto({ source: captura }, { caption, parse_mode: 'HTML' }).catch(() => {});
        else await ctx.replyWithHTML(caption).catch(() => {});

    } catch (err) {
        logTelcelPrueba(id, `❌ ${err.message || err}`);
        const captura = page && !page.isClosed() ? await page.screenshot({ fullPage: false }).catch(() => null) : null;
        const caption = `❌ <b>TELCEL PRUEBA — FALLA</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n<code>${String(err.message || err).slice(0,180)}</code>`;
        if (captura) await ctx.replyWithPhoto({ source: captura }, { caption, parse_mode: 'HTML' }).catch(() => {});
        else await ctx.replyWithHTML(caption).catch(() => {});
    } finally {
        await cerrarNavegadorTelcelPrueba(id).catch(() => {});
        const st = sesiones.get(id);
        if (st && st.paso !== 'numero') sesiones.delete(id);
    }
}
...
async function mostrarMenuInicio(ctx, esReinicio = false) {
    ...
    if (cerrarNavegadorTelcelPrueba) await cerrarNavegadorTelcelPrueba(id).catch(() => {});
    await limpiarMensajesTemporales(ctx, id);
    sesiones.delete(id);
...
bot.command(['telcelprueba', 'pruebacom'], async ctx => {
...
async function iniciarCompania(ctx, compania) {
    ...
    if (compania === 'Bait') {
        ...
        if (cerrarNavegadorTelcelPrueba) cerrarNavegadorTelcelPrueba(id);
    ...
    const modo = compania === 'Telcel Prueba'
        ? 'telcel_prueba'
        : ((compania === 'Telcel.com' || compania === 'Telcel Tienda') ? 'telcel_tienda' : compania.toLowerCase());
