...
async function flujoTelcelPrueba(ctx, id, datos) {
    ...
    try {
        ...
        // PASO A: paquete exacto por tarjeta visual confirmada en el DOM.
        // La página puede cargar/lazy-renderizar las tarjetas conforme hacemos scroll.
        logTelcelPrueba(id, `🔎 Buscando paquete Recarga ${monto}`);
        const tarjetaPaquete = page.locator('div.packages-grid__item')
            .filter({ hasText: new RegExp(`Recarga\s*${monto}\b`, 'i') })
            .first();

        let paqueteVisible = false;
        for (let intentoScroll = 1; intentoScroll <= 12; intentoScroll++) {
            paqueteVisible = await tarjetaPaquete.isVisible({ timeout: 500 }).catch(() => false);
            if (paqueteVisible) break;

            const existeEnDom = (await tarjetaPaquete.count().catch(() => 0)) > 0;
            if (existeEnDom) {
                logTelcelPrueba(id, `⬇️ Paquete localizado en DOM; desplazando hacia Recarga ${monto}`);
                await tarjetaPaquete.scrollIntoViewIfNeeded().catch(() => {});
                await page.waitForTimeout(350);
                paqueteVisible = await tarjetaPaquete.isVisible({ timeout: 1500 }).catch(() => false);
                if (paqueteVisible) break;
            } else {
                logTelcelPrueba(id, `⬇️ Scroll de búsqueda ${intentoScroll}/12 para localizar Recarga ${monto}`);
                await page.evaluate(() => window.scrollBy(0, Math.max(450, Math.floor(window.innerHeight * 0.75)))).catch(() => {});
                await page.waitForTimeout(350);
            }
        }
